...
import { Clock, Users, ChefHat, Trash2, Loader2, RefreshCw, Star, StickyNote, Menu, Printer, ListPlus, ListCheck, Share2 } from "lucide-react";
...
export function MyRecipesView({ userId = null }: MyRecipesViewProps) {
  ...
  const handleShare = async (recipe: SavedRecipe) => {
    ...
    setShareUrl(null);
    setShareRecipeName(recipe.name);
    ...
    const result = await shareRecipe(recipe, userName);
  ...
  const filteredRecipes = selectedCategory === "All" ? recipes : recipes.filter((recipe) => recipe.category.toLowerCase() === selectedCategory.toLowerCase());
  ...
  if (recipes.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-muted-foreground mb-6">Start planning meals and save your favorite recipes to see them here.</p>
      </div>
    );
  }
    ...
    <div>
            ...
            {categories.map((category) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  selectedCategory === category ? "bg-emerald-600 text-white shadow-md" : "bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700"
                }`}
              >
                {category}
      ...
      {filteredRecipes.length === 0 && recipes.length > 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mb-6">You don't have any saved recipes in this category yet.</p>
        </div>
      ) : (
              ...
              <CardHeader className="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950 dark:to-teal-950">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-start gap-2 mb-2">
                      {shoppingListItems.has(recipe.meal_id) && <ListCheck className="h-5 w-5 text-emerald-600 dark:text-emerald-400 flex-shrink-0 mt-0.5" />}
                  ...
                  <Popover open={openPopoverId === recipe.id} onOpenChange={(open) => setOpenPopoverId(open ? recipe.id : null)}>
                    ...
                    <PopoverContent className="w-56 p-2" align="end">
                      <div className="space-y-1">
                        <button onClick={() => handleToggleShoppingList(recipe)} className="w-full flex items-center gap-3 px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors rounded-md text-left">
                        ...
                        <button onClick={() => handleOpenNotesDialog(recipe)} className="w-full flex items-center gap-3 px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors rounded-md text-left">
                          <StickyNote className="h-4 w-4" />
                          <span>Notes / Rating</span>
                        </button>
                        <button onClick={() => handleShare(recipe)} className="w-full flex items-center gap-3 px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors rounded-md text-left">
                          <Share2 className="h-4 w-4" />
                          <span>Share</span>
                        </button>
                        <button onClick={() => handlePrint(recipe)} className="w-full flex items-center gap-3 px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors rounded-md text-left">
                ...
                <div className="flex gap-4 mt-3 text-sm">
                  ...
                  {recipe.rating && (
                    <div className="flex items-center gap-1">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`h-4 w-4 ${i < recipe.rating! ? "fill-yellow-400 text-yellow-400" : "fill-gray-200 text-gray-200 dark:fill-gray-700 dark:text-gray-700"}`} />
                      ))}
                    </div>
                  )}
                </div>
                ...
                {recipe.notes && (
                  <div className="mt-3 p-2 bg-white/50 dark:bg-gray-800/50 rounded text-xs text-muted-foreground italic border-l-2 border-emerald-500">
                    <span className="block truncate">
                      "{recipe.notes.substring(0, 50)}
                      {recipe.notes.length > 50 ? "..." : ""}"
                    </span>
                  </div>
                )}
              </CardHeader>
      ...
      {/* Notes Dialog */}
      {notesDialogRecipe && <RecipeNotesDialog recipe={notesDialogRecipe} open={!!notesDialogRecipe} onOpenChange={(open) => !open && setNotesDialogRecipe(null)} onUpdate={handleUpdateRecipe} />}

      {/* Share Dialog */}
      <ShareDialog isOpen={isShareDialogOpen} onClose={() => setIsShareDialogOpen(false)} shareUrl={shareUrl} isGenerating={isGeneratingShare} recipeName={shareRecipeName} />
    </div>
  );
}
