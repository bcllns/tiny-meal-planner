import { User, ChefHat, LogOut, Sparkles, UserPlus } from "lucide-react";
...
export function Header({ userName, userEmail, onSignOut, activeTab = "dashboard", onTabChange, showGetStarted = false }: HeaderProps) {
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            {showGetStarted ? (
              ...
              <div className="flex items-center gap-3">
                {/* Invite Friends Button - Only show for authenticated users */}
                {onSignOut && (
                  <Button variant="outline" size="sm" onClick={() => setIsInviteDialogOpen(true)} className="hidden sm:flex items-center gap-2">
      ...
      <InviteFriendsDialog isOpen={isInviteDialogOpen} onClose={() => setIsInviteDialogOpen(false)} />
    </>
  );
}
