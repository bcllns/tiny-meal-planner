...
export async function generateMealPlan(numberOfPeople: number, mealType: string = "all", notes: string = ""): Promise<Meal[]> {
  ...
  const notInterestedRecipes = notInterestedResult.success && notInterestedResult.data ? notInterestedResult.data.map((recipe) => recipe.recipe_name) : [];
  ...
  console.log("Meal plan generation response data:", data);
