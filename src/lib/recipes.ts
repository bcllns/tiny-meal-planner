...
export async function saveRecipe(meal: Meal): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    // Use the existing meal.id instead of generating a new UUID
...
export async function checkIfRecipeSaved(mealId: string): Promise<boolean> {
  ...
  try {
    const { data, error } = await supabase.from("recipes").select("id").eq("meal_id", mealId).maybeSingle();
...
export async function getSavedRecipes(): Promise<SavedRecipe[]> {
  ...
  try {
    ...
    const { data, error } = await supabase.from("recipes").select("*").eq("user_id", currentUser.id).order("name", { ascending: true });
...
export async function deleteRecipe(mealId: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    const { error } = await supabase.from("recipes").delete().eq("meal_id", mealId);
...
export async function updateRecipeNotesAndRating(recipeId: string, notes: string | null, rating: number | null): Promise<{ success: boolean; error?: string }> {
  if (!supabase) {
    return { success: false, error: "Supabase is not configured" };
  }

  try {
    const { error } = await supabase.from("recipes").update({ notes, rating }).eq("id", recipeId);
