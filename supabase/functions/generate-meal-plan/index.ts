 ...
 */
async function saveOpenAIResults(supabaseClient: any, userId: string, meals: any[]): Promise<void> {
  try {
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { numberOfPeople, mealType, notes, notInterestedRecipes, savedRecipeNames } = await req.json();
...
[
  {
    ...
    try {
      ...
      saveOpenAIResults(supabaseClient, user.id, meals).catch(err => {
